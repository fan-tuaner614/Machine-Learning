...
def load_and_process_data(file_path):
    ...
    try:
        ...
        print(f"错误: 找不到数据文件 '{file_path}'。请确保它和脚本在同一个目录下。")
        return None, None
...
class KNN:
    def __init__(self, k=5, verbose=True):
        self.k = k
        if verbose:
            print(f"kNN分类器已创建，k值为 {self.k}。")

    def fit(self, X_train, y_train, verbose=True):
        self.X_train = X_train
        self.y_train = y_train
        if verbose:
            print("模型'训练'完成（数据已存储）。")
...
def plot_metrics_separately(k_values, metrics_dict):
    ...
    fig.suptitle('kNN模型评估指标随 k 值的变化曲线 (基于LOOCV)', fontsize=20, y=1.0)
    ...
    if features is not None:
        ...
        for k in k_candidates:
            ...
            cen_score = calculate_cen(all_true_labels, all_predicted_labels) # 调用新函数
